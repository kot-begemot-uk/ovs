...
{
    return rpc->status ? 0 : rpc->data.backlog;
}
...
{
    if (rpc->status || !byteq_is_empty(&rpc->data.input)) {
        poll_immediate_wake_at(rpc->name);
    } else {
        stream_recv_wait(rpc->data.stream);
    }
}
